...
Вначале создадим интерфейс *IInjectedChannelNotifare* для уведомления класса  *ResistanceThermometers* о том что пришли данные. Класс *ResistanceThermometers*, отвечает за преобразования кода с АЦП с инжекторного канала в температуру, полученного из класса *InjectedChannel* с помощью агрегации интерфейса *IDataProvider*.
...
Класс *ResistanceThermometers*, разбит на заголовочный файл .h и файл исходного кода .cpp.

.resistancethermometers.h
[source, cpp]
----
